...
\begin{document}
	...
	\subsubsection{Нахождение точек бифуркации}
	
	Рассмотрим систему с параметром \(\varepsilon\):
	\begin{equation}
		\frac{dx}{d\tau} = T \cdot f(x, \varepsilon), \quad x(1) = x(0), \quad \tau \in [0,1],
	\end{equation}
	где \(T\) — период, \(\tau = t/T\) — нормированное время. Система решается совместно с уравнениями для матрицы монодромии \(U(\tau)\):
	\begin{equation}
	...
	Мультипликаторы матрицы \(U(1)\) определяют орбитальную устойчивость. Рассмотрим случаи бифуркаций:
	
	\paragraph{Бифуркация удвоения периода.} Один из мультипликаторов \(U(1)\) равен \(-1\). Для собственного вектора \(v\):
	\begin{equation}
		(U(1) + E) \cdot v = 0, \quad v^{(k)} = 1,
	\end{equation}
	где \(E\) — единичная матрица, \(v^{(k)}\) — \(k\)-я компонента \(v\). Это дает \(n+1\) уравнений для \(n+1\) неизвестных (\(v\), \(\varepsilon\)).
	
	\paragraph{Рождение (исчезновение) пары замкнутых траекторий.} Появляется мультипликатор, равный 1. Возможны два случая:
	
	1. Два линейно независимых собственных вектора \(v\) и \(w\):
	\begin{equation}
		(U(1) - E) \cdot v = 0, \quad (U(1) - E) \cdot w = 0.
	\end{equation}
	
	2. Один собственный вектор \(v\) и корневой вектор \(w\):
	\begin{equation}
		(U(1) - E) \cdot v = 0, \quad (U(1) - E) \cdot w = v.
	\end{equation}
	
	Для учета обеих ситуаций:
	\begin{equation}
		(U(1) - E) \cdot v = 0, \quad (U(1) - E)^2 \cdot w = 0, \quad v^{(k)} = 1, \quad w^{(k)} = 0.
	\end{equation}
	
	\paragraph{Возникновение инвариантного тора.} Модуль комплексно-сопряженной пары мультипликаторов \(\lambda_{1,2} = \alpha \pm i \omega\) равен 1. Для собственных векторов \(u \pm i v\):
	\begin{equation}
		U(1) \cdot (u + i v) = (\alpha + i \omega) \cdot (u + i v).
	\end{equation}
	Разделяя действительную и мнимую части:
	\begin{equation}
		(U(1) - \alpha E) \cdot u + \omega \cdot v = 0, \quad -\omega \cdot u + (U(1) - \alpha E) \cdot v = 0, \quad u^{(k)} = 1, \quad v^{(k)} = 0, \quad \alpha^2 + \omega^2 = 1.
	\end{equation}
	
	Решение систем для мультипликаторов совместно с уравнениями для \(U(\tau)\) позволяет строить бифуркационные диаграммы, варьируя \(\varepsilon\).
