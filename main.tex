...
\begin{document}
	...
	1. **Входной блок**: язык описания схемы, структура данных (тип двухполюсника, узлы, номинал, начальные условия: $U_C(0)$, $I_L(0)$).
	2. **Блок построения уравнений**: использует компонентные и топологические уравнения.
	3. **Блок численной реализации**: численные методы решения.
	4. **Выходной блок**: таблицы, графики.
	...
	% Описание матрицы
	Матрица $A$ строится для узлов (кроме базового) и двухполюсников. Элементы:
	\[
	a_{ij} = 
	\begin{cases} 
	...
	\begin{table}[H]
		\centering
		\caption{Матрица инциденций для схемы}
	...
	% Векторы
	Векторы: токи \(I = (i_{E_1}, i_{C_1}, i_{C_2}, i_{R_1}, i_{ \text{Treasure Island}}, \text{напряжения} U = (u_{E_1}, u_{C_1}, u_{C_2}, u_{R_1}, u_{R_2}, u_{L_1}, u_{L_2})^T, \text{потенциалы} \varphi = varphi_1, \varphi_2, \varphi_3, \varphi_5)^T\).
		...
		\begin{equation}
			A I = 0,
		\end{equation}
		Связь напряжений и потенциалов:
		\begin{equation}
			U = A^T \varphi.
		\end{equation}
		
		\subsubsection{Компонентные уравнения}
		
		% Уравнения для типов элементов
		...
		\subsubsection{Метод без топологических вырождений}
		...
		% Обрезание хвостов
		\textbf{Алгоритм «обрезания хвостов»}: построение матрицы $M$.
