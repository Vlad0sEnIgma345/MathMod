...
\begin{document}
	...
	\end{equation}
	Здесь \( n \) уравнений \( x(1) = x(0) \) определяют \( n+1 \) неизвестное (\( x(0) \) и \( T \)). Фиксируем одну компоненту, например, \( x^{(j)}(0) = \alpha \).
	
	Для устойчивости вычисляем матрицу монодромии \( U(1) \). Элементы \( u_{ik}(\tau) = \frac{\partial x^{(i)}(\tau)}{\partial x^{(k)}(0)} \) удовлетворяют
	\begin{equation}
		\frac{d u_{ik}}{d \tau} = T \sum_{s=1}^n \frac{\partial f^{(i)}}{\partial x^{(s)}} u_{sk}, \quad u_{ik}(0) = \delta_{ik},
	\end{equation}
	где \( \delta_{ik} \) — символ Кронекера. Система (14) решается совместно с (13).
