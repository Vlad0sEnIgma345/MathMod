...
\begin{document}
	...
	% Виды бифуркаций
	\subsection{Простейшие виды бифуркации периодического решения}
	...
	Для анализа выбирается сечение \( \Sigma \), а отображение \( P: \Sigma \to \Sigma \) сопоставляет \( x_0 \) следующую точку пересечения траектории с \( \Sigma \). Для предельного цикла \( P(x_0) = x_0 \). Бифуркации:
	- Рождение пары циклов: две неподвижные точки.
	- Удвоение периода: \( P^2(x_0) = x_0 \), \( P(x_0) \neq x_0 \).
	- Возникновение тора: инвариантная окружность.
	
	Сечение задаётся \( S(x) = 0 \). Вводим \( x^{(n+1)} = S(x) \) и дополняем систему
	\begin{equation}
		\frac{d x^{(n+1)}}{d t} = \sum_{j=1}^n \frac{\partial S}{\partial x^{(j)}} f^{(j)}(x).
	\end{equation}
	
	% Нахождение бифуркаций
	\subsection{Нахождение точек бифуркации для периодических решений}
	
	Рассмотрим систему с параметром \( \varepsilon \):
	\begin{equation}
		\frac{d x}{d \tau} = T f(x, \varepsilon), \quad x(1) = x(0).
	\end{equation}
	
	\textbf{Удвоение периода:} \( \rho = -1 \), условие
	\begin{equation}
		(U(1) + E) v = 0, \quad v^{(k)} = 1.
	\end{equation}
	
	\textbf{Рождение пары циклов:} два \( \rho = 1 \), условия
	\begin{equation}
		(U(1) - E) v = 0, \quad (U(1) - E)^2 w = 0, \quad v^{(k)} = 1, \quad w^{(k)} = 0.
	\end{equation}
	
	\textbf{Возникновение тора:} \( \lambda_{1,2} = \alpha \pm i \omega \), \( |\lambda| = 1 \),
	\begin{align}
		(U(1) - \alpha E) u + \omega v &= 0, \\
		-\omega u + (U(1) - \alpha E) v &= 0, \\
		u^{(k)} = 1, \quad v^{(k)} &= 0, \quad \alpha^2 + \omega^2 = 1.
	\end{align}
	
	% Итоги и задачи
	\subsection{Итоги и задачи анализа нелинейных моделей}
	
	Основные задачи:
	\begin{enumerate}
		\item Нахождение стационарных точек: \( f(x, \varepsilon) = 0 \).
		\item Анализ устойчивости: собственные значения \( \frac{\partial f}{\partial x} \).
		\item Диаграммы стационарных решений.
		\item Бифуркационные диаграммы.
		\item Нахождение периодических решений: (13).
		\item Анализ орбитальной устойчивости: (14).
		\item Диаграммы периодических решений.
		\item Бифуркационные диаграммы периодических решений: (17)--(19).
	\end{enumerate}
...
\textit{Засучите рукава, билет будет долгий}
\vspace{1em}

\textbf{Кратко:} Это одни большой в примерах, описывающий, как раскладывать решения любых уравнений (обычных и дифференциальных) по параметру в +- всех нестандартных случаях. \textbf{Судя по тому, что было на экзамене у прошлых групп, будет дан пример, который надо будет разложить по параметру, так что этот раздел изучается, к сожалению, целиком}

\subsection{Статическая модель. Алгебраические уравнения}

\subsubsection{Пример 1}
\begin{equation}
  f(x,\varepsilon) = x^2 - (3 + 2\varepsilon)x + 2 + \varepsilon = 0
  \label{eq:pertrubation_algebraic_init}
\end{equation}

\textbf{Возмущенное уравнение:} уравнение с $\varepsilon\ne0$

\textbf{невозмущенное (вырожденное) уравнение:} уравнение, где $\varepsilon = 0$

Заметим, что невозмущенное уравнение превращается в обычное квадратное, у которого 2 корня: $1$ и $2$

Разложение \eqref{eq:pertrubation_algebraic_init} будем искать в виде:

\begin{equation}
  x(\varepsilon) = \sum_{k=0}^\infty x_k \varepsilon^k = x_0 + x_1 \varepsilon + x_2 \varepsilon^2 + \ldots
  \label{eq:pertrubation_algebraic_expantion}
\end{equation}

\textit{В методичке валится с потолка. $x_1$, $x_2$, $x_3$, ... - не решения уравнения. Это просто какие-то коэффициенты. Поэтому фактически это просто степенной ряд}

Подставим \eqref{eq:pertrubation_algebraic_expantion} в \eqref{eq:pertrubation_algebraic_init}:

\begin{align*}
  &\left(x_0 + x_1 \varepsilon + x_2 \varepsilon^2 + \ldots\right)^2 - (3 + 2\varepsilon)\left(x_0 + x_1 \varepsilon + x_2 \varepsilon^2 + \ldots\right) + 2 + \varepsilon = 0, \\
\end{align*}

Приведем подобные при степенях $\varepsilon$

\begin{equation}
  \left(x_0^2 - 3x_0 + 2\right) + \varepsilon\left(2x_0x_1 - 3x_1 - 2x_0 + 1\right) + \varepsilon^2\left(2x_0x_2 + x_1^2 - 3x_2 - 2x_1\right) + \ldots = 0
\end{equation}

Самый простой способ добиться слева нуля - приравнять все слагаемые к нулю. $\varepsilon > 0$, значит приравнивать к нулю надо коэффициенты при степенях $\varepsilon$:
...
\begin{align*}
x^{(1)} &= 1 - \varepsilon + 3\varepsilon^2 + \ldots, \\
x^{(2)} &= 2 + 3\varepsilon - 3\varepsilon^2 + \ldots
\end{align*}

\textit{Заметим, что первыми слагаемыми в обоих ответах являются решения невырожденного уравнения. Это случилось потому что при $\varepsilon^0$ стоит исходное вырожденное уравнение. Это сохранится и далее}

\subsubsection{Пример 2}

\begin{equation}
  x^2 + (\varepsilon - 2)x + 1 = 0
  \label{eq:pertrubation_algebraic_init_2}
\end{equation}

Пробуем искать разложение в том же виде:

\begin{equation}
  \left(x_0 + x_1 \varepsilon + x_2 \varepsilon^2 + \ldots\right)^2 + (\varepsilon - 2)\left(x_0 + x_1 \varepsilon + x_2 \varepsilon^2 + \ldots\right) + 1 = 0, \\
\end{equation}

\begin{align*}
  \varepsilon^0: & \quad x_0^2 - 2x_0 + 1 = 0, \quad x_0^{(1)} = x_0^{(2)} = 1; \\
  \varepsilon^1: & \quad 2x_0x_1 - 2x_1 = 0 \quad \Rightarrow 1 = 0 \quad !!!
\end{align*}

Если разложение приводит к противоречию в самом мироздании, значит оно не годится. Заметим, что решение вырожденного уравнение - 2 кратных корня, равных 1. По аналогии с первым примером, при $\varepsilon^0$ стоит невырожденное уравнение

\vspace{1em}

Попробуем ввести некоторое возмущение в разложение $x(\varepsilon)$ размером в $\delta$, при чем такое, что при $\varepsilon\to0 \Rightarrow \delta \to 0$, тогда запишем $x(\varepsilon) = 1 + \delta$ (самая удобная форма для преобразований) и подставим в \eqref{eq:pertrubation_algebraic_init_2}

\begin{equation}
  x^2 + (\varepsilon - 2)x + 1 = 0 \quad \Rightarrow \quad (x - 1)^2 = -\varepsilon \cdot x, \quad \Rightarrow \quad \delta^2 = -\varepsilon(1 + \delta)
\end{equation}

"Отсюда видно, что $\delta$ при малых $\varepsilon$ ведет себя как $\sqrt{\varepsilon}$. Поэтому будем искать разложение решения \eqref{eq:pertrubation_algebraic_init_2} в виде":

\begin{equation}
  x(\varepsilon) = \sum_{k=0}^\infty x_k \varepsilon^{k/2} = x_0 + x_1 \sqrt{\varepsilon} + x_2 \varepsilon + \ldots
\end{equation}

Выполним подстановку:

\begin{equation}
  \left(x_0 + x_1 \sqrt{\varepsilon} + x_2 \varepsilon + \ldots\right)^2 + (\varepsilon - 2)\left(x_0 + x_1 \sqrt{\varepsilon} + x_2 \varepsilon + \ldots\right) + 1 = 0
\end{equation}

\begin{align*}
  \sqrt{\varepsilon}: & \quad x_0^2 - 2x_0 + 1 = 0, \quad x_0^{(1)} = x_0^{(2)} = 1; \\
  \varepsilon: & \quad 2x_0x_1 - 2x_1 = 0, \quad 0 = 0; \\
  \varepsilon^{3/2}: & \quad x_1^2 + 2x_0x_2 - 2x_2 = 0, \quad x_1^2 + 1 = 0, \quad x_1^{(1)} = i, \quad x_1^{(2)} = -i; \\
  \varepsilon^2: & \quad 2x_0x_3 + 2x_1x_2 + x_1 - 2x_3 = 0, \quad x_2^{(1)} = x_2^{(2)} = \frac{1}{2}.
\end{align*}

Итоговые разложения:

\begin{align*}
  x^{(1)}(\varepsilon) &= 1 + i \cdot \sqrt{\varepsilon} - \frac{\varepsilon}{2} + \ldots, \\
  x^{(2)}(\varepsilon) &= 1 - i \cdot \sqrt{\varepsilon} - \frac{\varepsilon}{2} + \ldots
\end{align*}
