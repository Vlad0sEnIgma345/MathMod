...
\begin{document}
	...
	Для множества в $\mathbb{R}^n$ пусть $N(\varepsilon)$ — минимальное число гиперкубов с ребром $\varepsilon$ для покрытия множества. Фрактальная размерность $D$:
	\begin{equation}
	...
	\textbf{Пример 1.} Точка: $N(\varepsilon) = 1$, $D = 0$.
	
	\textbf{Пример 2.} Отрезок длины $L$: $N(\varepsilon) = \frac{L}{\varepsilon}$, $D = 1$.
	
	\textbf{Пример 3.} Поверхность площадью $S$: $N(\varepsilon) = \frac{S}{\varepsilon^2}$, $D = 2$.
	
	\textbf{Пример 4.} Канторово множество: $\varepsilon = \left(\frac{1}{3}\right)^m$, $N(\varepsilon) = 2^m$,
	\begin{equation}
		D = \lim_{m \to \infty} \frac{\ln 2^m}{\ln 3^m} = \frac{\ln 2}{\ln 3} \approx 0.63.
	\end{equation}
	
	\textbf{Пример 5.} Кривая Коха: $\varepsilon = \left(\frac{1}{3}\right)^m$, $N(\varepsilon) = 3 \cdot 4^m$,
	\begin{equation}
		D = \lim_{m \to \infty} \frac{\ln (3 \cdot 4^m)}{\ln 3^m} = \frac{\ln 4}{\ln 3} \approx 1.26.
	\end{equation}
	
	\subsubsection{Аттрактор Лоренца}
	...
	Стационарные точки: первая — $x = y = z = 0$. Матрица Якоби:
	\begin{equation}
		J = \begin{pmatrix}
			-\sigma & \sigma & 0 \\
			r - z & -1 & -x \\
	...
	\end{equation}
	При $r < 1$ точка устойчива, при $r > 1$ — неустойчива.
	...
	При $r \in [24.06, 24.74]$ — три аттрактора (две точки и хаотический). При $r \in (24.74, 30.1)$ — хаотический аттрактор, $D = 2.06$. При $r > 30.1$ — чередование хаоса и периодичности.
